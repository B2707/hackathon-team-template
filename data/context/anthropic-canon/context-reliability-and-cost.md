...
- Multi-agent token multiplier: a multi-agent run burns ~15× the tokens of a single chat (a standard single agent ~4×), and parallelism saves wall-clock time but NOT tokens — each parallel subagent's results still land in the main context. Reserve multi-agent for high-value breadth-first work; keep interdependent/coding work single-agent (https://www.anthropiccertifications.com/courses/introduction-to-subagents/multi-agent-orchestration).
