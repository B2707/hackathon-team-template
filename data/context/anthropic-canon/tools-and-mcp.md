...
- Three concrete tool profiles: Research = Read/Grep/Glob (read-only); **Reviewer = Read/Grep/Glob + Bash** (Bash is needed to run `git diff` and inspect pending changes) but explicitly NO Edit/Write — a reviewer is NOT fully read-only; Modifier = Reviewer + Edit/Write (https://www.anthropiccertifications.com/courses/introduction-to-subagents/limiting-tool-access).
- Quantified cross-role rule: routing every cross-role need through the coordinator adds 2-3 hops (~+40% latency), so when ~85% of cases are simple, give the agent a narrow CONSTRAINED tool (e.g. `verify_fact`) for those and route only the ~15% complex cases through the coordinator — never hand it the full generic tool (https://www.anthropiccertifications.com/courses/claude-certified-architect-foundations/tool-distribution-and-tool-choice).
...
> Protocol internals and building MCP servers/clients (architecture roles, the
> JSON-RPC data layer, transport mechanics (stdio vs Streamable HTTP),
> connection lifecycle/capabilities, the three
> server + three client primitives, authorization, testing, production) live in
> the dedicated [mcp.md](mcp.md). This section stays scoped to harness-side
> integration: how *we* configure and choose MCP servers.
...
- Transports: beyond stdio (`command`/`args`/`env`), a server can be HTTP — `{"type":"http","url":"…","headers":{"Authorization":"Bearer ${GITHUB_TOKEN}"}}` — and expansion also supports a fallback form `${VAR:-default}`. Claude Code also has a third "local" scope (private to you within one project) alongside project `.mcp.json` and user `~/.claude.json` (https://www.anthropiccertifications.com/courses/claude-certified-architect-foundations/mcp-server-integration).
...
- Preferred order on a non-unique anchor: WIDEN the anchor (add surrounding text) or use replace-all FIRST; Read + Write is the LAST resort, because it pulls the whole file into context and burns tokens — don't reach for it on a small change (https://www.anthropiccertifications.com/courses/claude-certified-architect-foundations/built-in-tools).
