...
- Bound a headless run with `--max-turns N` (caps agentic turns so a job can't run away) and `--max-budget-usd` (caps spend). Distractors that do NOT exist: a `CLAUDE_HEADLESS=true` env var, a `--batch` flag, and a `/dev/null` stdin redirect (a hack, not the documented mode) — the real non-interactive mode is `-p`/`--print` (https://www.anthropiccertifications.com/courses/claude-certified-architect-foundations/cicd-integration).
