...
# "review" and "tests-touched" — a required check that never reports blocks
# every merge (see docs/RUNBOOKS.md "Bring the review bot online").
#
  ...
  rules: ((.rules // [])
    ...
    + [{
        ...
        parameters: {
          ...
          required_status_checks: [{context: "review"}, {context: "tests-touched"}]
        }
      }])
}' | gh api -X PUT "repos/$REPO/rulesets/$ruleset_id" --input - >/dev/null

echo "OK: 'review' + 'tests-touched' are now required to merge into main on $REPO"
